...
def _(mo):
    ...
    **What this is.** A small teaching model — four events on a solid-on-solid lattice — not
    the full strained GaN/AlN quantum-dot model of Budagosky and Garcia-Cristobal (2022). Its
    RHEED curve is a clearly labelled morphology proxy, not an electron-diffraction
    calculation, so every comparison below is qualitative by construction.
    """)
    return
...
def _(IMAGES, mo):
    ...
    # generated by the model. Credits repeat in docs/REFERENCES.md.
...
def _(mo):
    ...
    **Finite size is the binding limitation.** Detrended proxy amplitude remains strongly
    ...
    at 7x7 as a smoke-scale check and no amplitude claim is made from it. Period and phase are
            ...
            mo.accordion(
                {
                    "Canonical seeds, sizes, and measured runtimes": mo.md(
                        ...
                        "Measured on the development M4 Pro: the Ga/N comparison is about 16 s "
                        "with four workers; Figure 3 convergence through 64x64 is about 52 s "
                        "with three effective workers; the sequential 64/128/256 benchmark is "
                        "about 34 s. Runtime varies with load."
                    )
                }
            ),
            batch_form,
