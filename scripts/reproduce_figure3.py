...
def main() -> None:
    ...
    for axis, ratio in zip(axes, FIGURE3_NOMINAL_GA_N_RATIOS, strict=True):
        ...
        oscillation_metrics = rheed_oscillation_metrics(predicted_coverage, mean)
        notebook_traces.append(
            {
                ...
                "oscillation_metrics": oscillation_metrics.as_dict(),
            }
        )
    ...
    plt.close(figure)
    (RUN_DIR / "figure3_simulated_smoke.json").write_text(json.dumps(summaries, indent=2) + "\n")
